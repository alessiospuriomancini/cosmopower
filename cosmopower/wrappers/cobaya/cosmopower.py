...
class CosmoPower(BoltzmannBase):
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            raise LoggedError(self.log, f"Failed to parse cosmopower package file:\n{str(e)}")
    ...
    def must_provide(self, **requirements) -> dict:
        ...
        for q in required_quantities:
            ...
            if q in self.networks:
                ...
                if "sigma8z" in self.networks:
                    self.log.debug("fsigma8 is not provided, but sigma8(z) is.")
                ...
                if q in self.parser.computed_parameters or self.translate_param(q) in self.parser.computed_parameters:
        ...
        if len(self._networks_to_eval) == 0:
            raise RuntimeError("No networks are to be evaluated (maybe none were loaded?).")
    ...
    def calculate(self, state: dict, want_derived: bool = True, **params) -> bool:
        ...
        if "derived" not in state: state["derived"] = {}
        ...
        if "derived" in self._networks_to_eval:
            ...
            for i, param in enumerate(self.parser.computed_parameters):
                ...
                p = self.find_translation(param) if self.find_translation(param) in self.provides else param
                outputs[p] = data[i]
    ...
    def get_Hubble(self, z: Union[float, np.ndarray], units: str = "km/s/Mpc") -> np.ndarray:
        ...
        for quantity in self.current_state:
            ...
            if quantity == "Hubble":
                ...
                hz = { "h": self.current_state[quantity], "z" : self.current_state["z"] }
                break
        ...
        hz = itp.interp1d(hz["z"], hz["h"], fill_value = "extrapolate", bounds_error = None)
        return prefac * np.atleast_1d(hz(z))
    
    def get_angular_diameter_distance(self, z: Union[float, np.ndarray]) -> np.ndarray:
        # First we need to find the DA(z) function we can provide.
        daz = None
        if "angular_diameter_distance" in self.current_state:
            daz = itp.interp1d(self.current_state["z"], self.current_state["angular_diameter_distance"], fill_value = "extrapolate", bounds_error = None)
    ...
    def get_sigma8(self, z: Union[float, np.ndarray]) -> np.ndarray:
        ...
        if "sigma8z" in self.current_state:
            s8z = itp.interp1d(self.current_state["z"], self.current_state["sigma8"], fill_value = "extrapolate", bounds_error = None)
    ...
    def get_fsigma8(self, z: Union[float, np.ndarray]) -> np.ndarray:
        ...
        if "fsigma8" in self.current_state:
            fs8z = itp.interp1d(self.current_state["z"], self.current_state["fsigma8"], fill_value = "extrapolate", bounds_error = None)
            return np.atleast_1d(fs8z(z))
        elif "sigma8z" in self.current_state:
            fs8z = -(1.0 + self.current_state["z"]) * np.gradient(self.current_state["sigma8z"], self.current_state["z"])
            fs8z = itp.interp1d(self.current_state["z"], fs8z, fill_value = "extrapolate", bounds_error = None)
            return np.atleast_1d(fs8z(z))
        
        raise LoggedError(self.log, "Need either fsigma8(z) or sigma8(z) to compute fsigma8, but neither are computed.")
    
    def get_Cl(self, ell_factor: bool = False, units: str = "FIRASmuK2") -> dict:
        cls_old = self.current_state.copy()
        
        if not "ell" in cls_old:
            raise Exception(f"Parser file {self.parser.yaml_filename} does not have l modes defined.")
        ...
        for k in cls_old["Cl"]:
            ...
            cls[k][ls] = cls_old["Cl"][k] * prefac * cp.util.cmb_unit_factor(k, units, 2.7255)
            cls[k][:2] = 0.0
    ...
    def get_in_dict(self, dct: dict, path: str) -> Any:
        """
        Networks are labeled with paths like "Cl/tt".
        This function parses such a path and finds the
        correct value in a dictionary.
        ...
        # This can probably be done a lot cleaner.
        while len(path) > 1:
    ...
    def set_in_dict(self, dct: dict, path: str, value: Any) -> dict:
        ...
        # This can probably be done a lot cleaner.
        # Essentially, we need to recursively dig into this dictionary and check to create
        # empty dictionaries if the path doesn't yet exist.
        while len(path) > 1:
            new = tgt.get(path[0], { })
            tgt[path.pop(0)] = new
            tgt = new
    ...
    def get_can_provide(self) -> Iterable[str]:
        ...
        for network in self._networks:
            if network == "derived":
                continue
            self.set_in_dict(can_provide, network, self.networks[network].modes.max())
    ...
    def translate_param(self, p: str) -> str:
        """
        Checks if a parameter needs to be renamed, and returns the renamed param if so.
        """
